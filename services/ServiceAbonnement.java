...
import shopp.entities.Abonnement;
...
public class ServiceAbonnement {
    public Connection connection = new Connection();
     ...
     public void ajouter_ab(Abonnement p)
     {
        ...
        try {
            req = connection.getConnection().prepareStatement("insert into abonnement(name,email,num_tel)values(?,?,?)");
     ...
     req.execute();
      ...
      public ResultSet getall_ab() {
        ...
        try {
            PreparedStatement req = connection.getConnection().prepareStatement("SELECT * FROM abonnement");
    ...
    public boolean supprimer_ab(Abonnement p) {
        ...
        try {
            ...
            PreparedStatement req = connection.getConnection().prepareStatement("delete from abonnement where name = ? ");
            req.setString(1, p.getName());
            req.executeUpdate();
ok=true;
        } catch (SQLException ex) {
    ...
    public void modifier_ab(Abonnement p ,String nom) {
        try {
            PreparedStatement req = connection.getConnection().prepareStatement("update abonnement set name=?,email=? ,num_tel=? where name=?");
